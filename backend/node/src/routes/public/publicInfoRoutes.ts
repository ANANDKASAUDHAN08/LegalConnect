...
// 1. GET /api/info/help - Filter FAQs based on query & category
...
// 2. POST /api/info/help/:id/vote - Vote on FAQ helpfulness
router.post('/help/:id/vote', asyncHandler(async (req: Request, res: Response) => {
  const id = Array.isArray(req.params.id) ? req.params.id[0] : (req.params.id as string);
  const { helpful } = req.body;

  if (!id) {
    throw AppError.badRequest('FAQ ID is required.');
  }
...
// 3. POST /api/info/analytics/search - Search Analytics Telemetry
router.post('/analytics/search', asyncHandler(async (req: Request, res: Response) => {
...
// 4. GET /api/info/about - About Page Metrics
router.get('/about', asyncHandler(async (_req: Request, res: Response) => {
  res.json({
    success: true,
    stats: {
...
// 5. GET /api/info/terms - Terms of Service Summary
router.get('/terms', asyncHandler(async (_req: Request, res: Response) => {
  res.json({
    success: true,
...
// 6. GET /api/info/privacy - Privacy Policy Officer details
router.get('/privacy', asyncHandler(async (_req: Request, res: Response) => {
  res.json({
    success: true,
    lastUpdated: '20 JUL 2026',
...
// 7. POST /api/info/contact - Create a support ticket
...
// 8. GET /api/info/contact/track/:query - Track ticket by ID or Email
router.get('/contact/track/:query', asyncHandler(async (req: Request, res: Response) => {
  const rawQuery = req.params['query'];
  const query = (Array.isArray(rawQuery) ? rawQuery[0] : (rawQuery || '')).trim().toLowerCase();

  if (!query) {
    throw AppError.badRequest('Ticket ID or Email is required.');
  }

  const isExactTicketId = query.startsWith('lc-');

  const dbMatches = await ContactTicket.find({
    $or: [
  ...
  dbMatches.forEach(d => combinedMap.set(d.ticketId.toUpperCase(), {
    ...
    status: d.status,
    notes: d.notes || [],
    timestamp: d.timestamp || (d as any).createdAt,
...
// 9. POST /api/info/contact/followup - Add follow-up note to ticket
router.post('/contact/followup', asyncHandler(async (req: Request, res: Response) => {
  const { ticketId, note } = req.body;

  if (!ticketId || !note || !note.trim()) {
    throw AppError.badRequest('Ticket ID and note content are required.');
  }

  const cleanTicketId = ticketId.trim().toUpperCase();

  const ticketDoc = await ContactTicket.findOne({
  ...
  const existingNotes: any[] = ticketDoc.notes || [];
...
// 10. POST /api/info/contact/withdraw — Withdraw ticket under DPDP consent withdrawal
router.post('/contact/withdraw', asyncHandler(async (req: Request, res: Response) => {
  const { ticketId } = req.body;
  if (!ticketId) {
    throw AppError.badRequest('Ticket ID is required for withdrawal.');
  }
...
// 11. GET /api/info/helplines — Fetch emergency helplines
